...
public class LocomotionTeleport : MonoBehaviour
{
	...
	/// <summary>
	/// Created at runtime, this gameobject is used to track where the playerModel will teleport. 
	/// The actual position depends on the type of Aim Handler and Target Handler that is active.
	/// Aim Handlers:
	/// * Laser: playerModel capsule swept along aim ray until it hits terrain or valid target.
	/// * Parabolic: playerModel capsule swept along a series of line segments approximating a parabolic curve until it hits terrain or valid target.
	/// Target Handlers:
	...
	/// is good for ensuring the playerModel can't teleport through bars to get out of a jail cell. 
	/// </summary>
	...
	/// from the playerModel to the teleport destination.
	/// </summary>
	...
	/// 
	/// The transition handler is responsible for actually moving the playerModel to the destination, and can achieve
	/// this goal however it wants. Example teleport transition handlers include:
	/// * Instant - Just move the playerModel with no delay or effect.
	...
	/// is a transition handler that needs to take some time to move the playerModel to the new location.
	/// This allows transition handlers to take as much (or little) time as they need to complete their task
	/// of moving the playerModel to the teleport destination.
	/// </summary>
